/**
 * regenerate-all-visual.ts
 * Regenerates ALL articles (except top-10 already done) with the visual-first prompt.
 *
 * Usage: npx tsx --env-file=.env.local scripts/regenerate-all-visual.ts
 */
...
// Already regenerated with visual prompt — skip these
...
3. <ProsCons pros={["...", "...", "..."]} cons={["...", "..."]} />
4. ## Cuándo elegir [herramienta A] (3-4 bullets concretos)
5. ## Cuándo elegir [herramienta B] (3-4 bullets concretos)
6. <Callout type="tip">Consejo para pymes españolas con el contexto clave</Callout>
7. ## Precios comparados (tabla)
8. ## Preguntas frecuentes (3 preguntas H3)
9. CTAs con <AffiliateLink ... />`,
    'top-list': `
...
async function main() {
  ...
  const slugs = files.map(f => f.replace('.mdx', '')).filter(s => !ALREADY_DONE.has(s))

  console.log(`\n🎨  Regenerando ${slugs.length} artículos con formato visual (${ALREADY_DONE.size} ya hechos)\n`)
  ...
  for (let i = 0; i < slugs.length; i++) {
    ...
    try {
      ...
      if (msg.includes('credit balance')) {
        console.error('\n💳  Créditos agotados. Vuelve a ejecutar el script para continuar.\n')
        process.exit(1)
      }
    }
  }

  console.log('\n✅  Todos los artículos regenerados con formato visual\n')
}
